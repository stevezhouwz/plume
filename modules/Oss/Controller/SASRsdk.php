...
function sendvoice($secretKey, $SecretId, $EngSerViceType, $SourceType, $URI, $VoiceFormat)
{
    if (empty($secretKey)) {
    ...
    $query_arr['UsrAudioKey'] = randstr(16);

    ksort($query_arr);

    $signStr = formatSignString('aai.tencentcloudapi.com', '/', $query_arr, 'POST');
    ...
    $rsp_str = "";
    $starttime = time();
    $ret = http_curl_exec($url, $query_arr, $rsp_str, $http_code, 'POST', 10, array(), $headers);
    //echo "ret : ".$ret."\n";
